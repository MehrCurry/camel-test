...
final class MyRouteBuilder extends RouteBuilder {
    ...
    public void configure() {
        from("file:data/inbox/?noop=true&delay=10000").to("log:de.gzockoll.prototype.camel").multicast()
                .to("seda:process").to("seda:audit").end();

        from("ftp://ftpuser@zockoll.dyndns.org/in?password=tux88.").to("log:de.gzockoll.prototype.camel").multicast()
                .to("seda:process").to("seda:audit").end();
        ...
        from("seda:process")
                ...
                .convertBodyTo(Ueberweisung.class).to("log:de.gzockoll.prototype.camel").to("mock:data");
