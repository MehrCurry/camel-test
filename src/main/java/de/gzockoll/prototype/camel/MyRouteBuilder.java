...
final class MyRouteBuilder extends RouteBuilder {
    ...
    public void configure() {
        ...
        from("quartz://myGroup/seconds?cron=*+*+*+*+*+?").setBody(
                constant(new SimpleObservation("Test3", new DateTime().getSecondOfMinute()))).to("seda:observations");

        from("quartz://myGroup/myTestTimerNameX?cron=*/10+*+*+*+*+?")
                .setBody(constant(InstrumentConfiguration.builder().name("Test3").title("JUnit").max(40).build()))
        ...
        from("activemq:topic:observations").aggregate(constant(true), new ArrayListAggregationStrategy())
                .completionInterval(2000L).eagerCheckCompletion().process(new CountProcessor("Messages", 1))
                .to("log:de.gzockoll.prototype.camel?showAll=true&multiline=true").to("seda:observations");
