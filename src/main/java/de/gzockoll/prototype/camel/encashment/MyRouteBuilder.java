...
public final class MyRouteBuilder extends RouteBuilder {
    ...
    public void configure() {
        errorHandler(deadLetterChannel("seda:error").maximumRedeliveries(5).retryAttemptedLogLevel(LoggingLevel.WARN)
                .redeliveryDelay(250).backOffMultiplier(2));

        from("quartz://myGroup/myTimerName/*/*/*/*/*/$").bean(EncashmentService.class, "startProcessing()");

        from("direct:input").to("seda:inkasso1");

        from("seda:inkasso1").multicast().to("seda:filemanager").choice()
        ...
        from("seda:inkasso1_order").to("ftp://ftpuser@fritz.box/out?password=tux88.");

        from("seda:inkasso1_credit").setHeader("subject", constant("Credit received"))
                .setHeader("to", constant("gzockoll@gmail.com")).to("seda:gmail");

        from("seda:inkasso1_payment").setHeader("subject", constant("PAYMENT made"))
                .setHeader("to", constant("gzockoll@gmail.com")).to("seda:gmail");
